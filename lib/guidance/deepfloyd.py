...
class IF(nn.Module):
    def __init__(self, device, vram_O, t_range=[0.02, 0.98]):
        ...
        # Create model
        pipe = IFPipeline.from_pretrained(model_key,
                                          variant="fp16",
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, grad_scale=1):
        ...
        images = F.interpolate(pred_rgb, (self.res, self.res), mode='bilinear', align_corners=False) * 2 - 1
        ...
        with torch.no_grad():
            ...
            noise_pred = self.unet(model_input.type(self.precision_t), tt, encoder_hidden_states=text_embeddings).sample
        ...
        loss = 0.5 * F.mse_loss(images, target, reduction="sum")
    ...
    def prompt_to_img(self, prompts, negative_prompts='', height=512, width=512, num_inference_steps=50,
        ...
        # Text embeds -> img
        imgs = self.produce_imgs(text_embeds, height=height, width=width, num_inference_steps=num_inference_steps,
...
if __name__ == '__main__':
    ...
    subjects = open("./data/prompt/fictional.txt", 'r').read().splitlines()[7:8]
    opt.negative = "lowres, bad anatomy, low quality, supersaturation, blurry"
    imgs = [
        np.vstack([
            sd.prompt_to_img(f"a {v} 3D rendering of {prompt}, full-body", opt.negative, opt.H, opt.W, opt.steps)[0]
            for v in [
