...
def forward_flex(self, x):
    ...
    pos_embed = self._resize_pos_embed(
        self.pos_embed, torch.div(h, self.patch_size[1], rounding_mode='floor'),
...
def _make_encoder(backbone, features, use_pretrained, groups=1, expand=False, exportable=True, hooks=None,
                  use_vit_only=False, use_readout="ignore", ):
...
class DepthNormalEstimation():
    def __init__(self, use_depth=False, device='cuda'):
        ...
        if self.use_depth:
            path = 'data/omnidata/omnidata_dpt_depth_v2.ckpt'
            self.depth_model = self.load_(path, DPTDepthModel(backbone='vitb_rn50_384'))

        path = 'data/omnidata/omnidata_dpt_normal_v2.ckpt'
        self.normal_model = self.load_(path, DPTDepthModel(backbone='vitb_rn50_384', num_channels=3))

    def load_(self, path, model):
    ...
    def __call__(self, image):
        ...
        """
        H, W = image.shape[2:]
        image = F.interpolate(image, size=self.img_size, mode='bicubic', align_corners=False)

        normal = self.normal_model(image).clamp(0, 1)
        normal = F.interpolate(normal, size=(H, W), mode='bicubic', align_corners=False)

        if self.depth_model is not None:
            depth = self.depth_model(image * 2 - 1).clamp(0, 1)
