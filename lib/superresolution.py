...
class SuperresolutionHybrid8XDC(torch.nn.Module):
    def __init__(self, channels, input_resolution=512, img_resolution=1024, sr_num_fp16_res=1, sr_antialias=True,
                 **block_kwargs):
        super().__init__()
        # assert img_resolution == 512
        ...
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlock(channels, 256, w_dim=img_resolution, resolution=img_resolution // 2,
                                     img_channels=3, is_last=False, use_fp16=use_fp16,
                                     conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        # self.block1 = SynthesisBlock(256, 128, w_dim=512, resolution=img_resolution,
        #                              img_channels=3, is_last=True, use_fp16=use_fp16,
        #                              conv_clamp=(256 if use_fp16 else None), **block_kwargs)
